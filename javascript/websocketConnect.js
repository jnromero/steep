...
if(pageName=="client.html"){
  window.subjectID = getUrlParameter('subjectID');
  window.viewType = getUrlParameter('viewType');
}
else if(pageName=="video.html" || pageName=="video.py"){
  window.viewType = "video";
  window.subjectID = "video";
}
else if(pageName=="monitor.html" || pageName=="monitor.py"){
  window.viewType = "monitor";
  window.subjectID = "monitor";
}
else{
  window.viewType = "unknown";
  window.subjectID = "unknown";
}
...
window.onload = function() {
  ...
  if (sock) {
    sock.onopen = function() {
      var message={"type":"newConnection","subjectID":window.subjectID,"viewType":window.viewType};
      sendMessage(message);
    }
...
function connectAnotherBrowserMessage(message){
    clearAll();
...
function notAcceptingMessage(message){
